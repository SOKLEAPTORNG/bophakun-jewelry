...
const CartScreen = props => {
  ...
  const cartList = async () => {
    ...
    const cartItemSort = transformedCartItems.sort((a, b) =>
      a.productId < b.productId ? 1 : -1,
    );
    //maco
    calculate(cartItemSort);
    console.log('reset cartItems');
   ...
   */
  const calculate = cartItemSort => {
    // alert(1);
    let price_percentage = 0;
    let total_discount = 0;
    let discount_percentage = 0;
    let discount_fixed = 0;
    let price_amount_after_discount = 0;
    let totalBeforeDiscount = 0;
    let totalAfterDiscount = 0;

    if (promotions.length > 0) {
      for (let i = 0; i < cartItemSort.length; i++) {
        console.log('cartItemSort', cartItemSort);
        console.log('cartItem', cartItem);
        const cat_id = cartItemSort[i].cat_id;
        const qty = cartItemSort[i].quantity;
        const sum = cartItemSort[i].sum;
        totalBeforeDiscount = Number(totalBeforeDiscount) + Number(sum);
        promotions.map(promo => {
          ...
          if (promo.category_id === cat_id) {
            if (promo.discount_type === 'fixed') {
              ...
              // price_amount_after_discount = sum - discount_amount;
              let convert2fix = discount_fixed.toFixed(2);
              totalAfterDiscount = totalAfterDiscount + Number(convert2fix);
            } else {
              total_discount = discount_amount / 100;
              price_percentage = sum * total_discount;
              // price_amount_after_discount =
              // Number(sum).toFixed(2) - price_percentage;
              let convert2fix = price_percentage.toFixed(2);
              totalAfterDiscount = totalAfterDiscount + Number(convert2fix);
          ...
          setTotalAmount(totalBeforeDiscount - totalAfterDiscount);
      ...
      for (let i = 0; i < cartItemSort.length; i++) {
        ...
        const qty = cartItemSort[i].quantity;
        const sum = cartItemSort[i].sum;
        totalAmount = Number(totalAmount) + Number(sum);
      }
  ...
  useEffect(() => {
    ...
    cartList();
  }, [loading, cartTotalAmount]); //when cart total amount was trigger or change value will effect calculate functions | recalculate again
