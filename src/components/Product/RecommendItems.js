...
const RecommmentItems = ({
  ...
  const [availablePromotionsState, setAvailablePromotionsState] = useState(
    promotion.filter(d => d.category_id === cat_id),
  );
  const available_promotions = promotion.filter(d => d.category_id === cat_id);
  // setAvailablePromotionsState(available_promotions);
  ...
  useEffect(() => {
    if (availablePromotionsState.length > 0) {
      availablePromotionsState.map(d => {
        let discount;
        ...
        <View
          ...
          <View style={{position: 'absolute', bottom: 5, left: 8}}>
            <Text style={{fontSize: 11}}>
              {availableStatus} | {i18n.t('home.Sold')} {(qtySoldItemResult * 1).toFixed(0)}
            </Text>
          </View>
        </View>
      </TouchableCmp>
...
export const styles = StyleSheet.create({
  ...
  container: {
    overflow: 'hidden',
    width:
      Platform.OS === 'ios' ? SCREEN_WIDTH / 3 - 10 : SCREEN_WIDTH / 3 - 10,
    height: 160,
